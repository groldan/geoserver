...
public class WorkspaceAdminRestIntegrationTest extends WorkspaceAdminCatalogRESTTestSupport {
    ...
    public void testWorkspaceSpecificTemplates() throws Exception {
        ...
        // Try in unauthorized workspace
        // TODO: TemplateController writes directly to data directory without SecureCatalog check,
        // so this currently succeeds (201) instead of returning 403
        response = putAsServletResponse(
    ...
    public void testListResources() throws Exception {
        ...
        assertEquals(200, getAsServletResponse("/rest/resource?format=json").getStatus());
        assertEquals(
                200,
                getAsServletResponse("/rest/resource/workspaces?format=json").getStatus());
    }
    ...
    public void testAccessWorkspaceResources() throws Exception {
        ...
        MockHttpServletResponse response =
                getAsServletResponse("/rest/resource/workspaces/%s?format=json".formatted(WS));
        assertEquals(200, response.getStatus());

        // Unauthorized workspace -> hidden -> 404
        response = getAsServletResponse("/rest/resource/workspaces/%s?format=json".formatted(WS_OTHER));
        assertEquals(404, response.getStatus());
    }
    ...
    public void testCreateWorkspaceResource() throws Exception {
        ...
        MockHttpServletResponse response = putAsServletResponse(
                "/rest/resource/workspaces/%s/test.txt".formatted(WS), content, MediaType.TEXT_PLAIN_VALUE);
        assertEquals(201, response.getStatus());

        // Read it back
        response = getAsServletResponse("/rest/resource/workspaces/%s/test.txt".formatted(WS));
        assertEquals(200, response.getStatus());
        assertEquals(content, response.getContentAsString());

        // Try in unauthorized workspace
        response = putAsServletResponse(
                "/rest/resource/workspaces/%s/test.txt".formatted(WS_OTHER), content, MediaType.TEXT_PLAIN_VALUE);
        // TODO: should be 403 but SecureResourceStore.out() throws IllegalStateException -> 500
        assertThat("Should deny write to other workspace", response.getStatus(), Matchers.oneOf(403, 500));
    }
    ...
    public void testWorkspaceSettings() throws Exception {
        ...
        // Unauthorized workspace settings
        // TODO: LocalSettingsController doesn't check workspace exists -> NPE -> 500
        response = getAsServletResponse("/rest/workspaces/%s/settings.xml".formatted(WS_OTHER));
        assertThat(
                "Should deny access to unauthorized workspace settings",
                response.getStatus(),
                Matchers.oneOf(403, 404, 500));
    }
