...
public class CatalogSecurityFilterBuilder {
    ...
    /**
     * @return {@link Filter#INCLUDE} if {@code names} is empty, {@code not(equalOrIn(nameProperty, names)} otherwise
     */
    ...
    /**
     * @return {@link Filter#INCLUDE} if {@code names} is empty, {@code defaultIfAny} if {@code names} contains
     *     {@code *}, a "{@code name = names.get(0)}" filter if {@code names} has a single element, a "{@code name
     *     IN(names)}" filter if {@code names} has multiple elements.
     */
