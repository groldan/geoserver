...
public class AccessManagerConfig implements Serializable, Cloneable {
    ...
    /**
     * Use authenticated users roles to match rules, in addition to username (defaults to {@code true})
     *
     * @return {@code true} if {@link Authentication} role names are included in {@link AccessRequest} and
     *     {@link AdminAccessRequest} for the {@link AuthorizationService} (default behavior), {@code false} if only
     *     authenticated user names shall be used.
     */
     ...
     * Use authenticated users roles to match rules, in addition to username (defaults to {@code true}).
     *
     * @param {@code true} if {@link Authentication} role names are to be included in {@link AccessRequest} and
     *     {@link AdminAccessRequest} for the {@link AuthorizationService} (default behavior), {@code false} if only
     *     authenticated user names shall be used.
     */
    ...
    /**
     * List of mutually exclusive roles used for rule matching when {@link #setUseRolesToFilter useRolesToFilter} is
     * {@code true}
     *
     * <p>By default all roles are accepted and this property equals to the {@code *} wildcard character.
     *
