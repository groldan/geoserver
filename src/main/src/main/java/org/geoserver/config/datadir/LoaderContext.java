...
class LoaderContext {
    ...
    /**
     * Thread factory used to load {@link CatalogInfo}. A short lived
     * {@link ForkJoinPool} will be created with this factory and the configured
     * {@link #DEFAULT_PARALLELISM parallelism}
     */
    private static final ForkJoinWorkerThreadFactory INITIALLIZATION_THREAD_FACTORY = new ForkJoinWorkerThreadFactory() {
        private AtomicInteger threadIdSeq = new AtomicInteger();

        public @Override ForkJoinWorkerThread newThread(ForkJoinPool pool) {
            String name = String.format("ForkJoinPool.DataDirectoryLoader-%d", threadIdSeq.incrementAndGet());
    ...
    private static final int DEFAULT_PARALLELISM = Math.max(2, Runtime.getRuntime().availableProcessors() / 2);
    ...
    public LoaderContext(CatalogImpl catalog, XStreamPersisterFactory persisterFactory) {
        ...
        final UncaughtExceptionHandler uncaughtExceptionHandler = (thread, exception) -> {
            LOGGER.log(Level.SEVERE, "Uncaught exception loading data directory at thread " + thread.getName(),
                    exception);
        };
        final boolean asyncMode = false;
        PROCESSING = new ForkJoinPool(parallelism, INITIALLIZATION_THREAD_FACTORY, uncaughtExceptionHandler, asyncMode);
    }
    ...
    public XStreamPersister persister() {
        ...
        if (xp == null) {
            ...
            System.err.printf("######### Created XStreamPersister for %s%n", Thread.currentThread().getName());
